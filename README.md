...
![聊天界面演示](./docs/demo-chat.png)

*实际运行截图：真 token 级流式输出 + 引用来源溯源 + 分点排版。启动方式见 [快速开始](#快速开始)。*
...
- **单元测试**：92 passed 1 skipped（工具数据流、缓存准入、Supervisor 路由回环、并发控制限流/防击穿、长期记忆准入判断、技能库装配与容错、成本记账与解析）
